...
# Image Validation
def validate_jpg_image(value):
    if imghdr.what(value) != 'jpeg':
        raise ValidationError("Only JPG images are allowed.")

def validate_image_size(image):
    img = Image.open(image)
    width, height = img.size
    if width != 700 or height != 360:
        raise ValidationError(f"Image dimensions should be 700x360 pixels. Current dimensions: {width}x{height}.")
...
class Product(models.Model):
    ...
    product_image_1 = models.ImageField(
        upload_to="shop/product_image", blank=True, null=True, validators=[validate_jpg_image]
    )
